...
interface PostDao {
    ...
    @Query("select * from databasephotos where id = :id")
    fun getPhotosById(id: Int): LiveData<List<DatabasePhotos>>

    @Query("select * from databasephotos")
    fun getAllPhotos(): LiveData<List<DatabasePhotos>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insertAllPhotos(vararg posts: DatabasePhotos)
}

@Database(entities = [DatabasePost::class, DatabaseComment::class, DatabasePhotos::class], version = 322, exportSchema = false)
